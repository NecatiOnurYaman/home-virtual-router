...
[ -d /etc/home-virtual-router ] || { echo "error: create the empty project config directory first: sudo install -d -m 0750 /etc/home-virtual-router" >&2; exit 1; }
temporary="$(mktemp -d)"
cleanup() { rm -f -- "$temporary/router.env" "$temporary/allow-physical-deployment"; rmdir "$temporary" 2>/dev/null || true; }
trap cleanup EXIT
...
touch "$temporary/allow-physical-deployment"
unshare --mount --net --pid --fork --mount-proc \
  "$script_dir/test-simulation-inner.sh" "$repo_dir" "$temporary"
