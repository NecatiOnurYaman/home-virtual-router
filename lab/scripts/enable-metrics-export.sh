...
if [ "$DEPLOYMENT_MODE" = "physical" ]; then
  ...
  "${ROUTER_CONTEXT_PREFIX[@]}" ip link show dev "$ROUTER_LAN_INTERFACE" >/dev/null || die "physical LAN interface is absent"
  "${ROUTER_CONTEXT_PREFIX[@]}" ip link show dev "$ROUTER_WAN_INTERFACE" >/dev/null || die "physical WAN interface is absent"
else
...
if [ "$DEPLOYMENT_MODE" = "physical" ] && [ "$PHYSICAL_TELEMETRY_INTERFACE" != none ]; then
  "${ROUTER_CONTEXT_PREFIX[@]}" ip link show dev "$PHYSICAL_TELEMETRY_INTERFACE" >/dev/null || die "physical telemetry interface is absent"
...
router_context_prefix
prefix=("${ROUTER_CONTEXT_PREFIX[@]}" env "PYTHONPATH=$HVR_REPO_DIR")
