...
for required_command in ip nft sysctl softflowd systemctl; do
...
if [ -e "$IPFIX_PID_FILE" ]; then
  stop_project_process_if_present "$IPFIX_PID_FILE" softflowd \
    "$IPFIX_COLLECTOR_HOST:$IPFIX_COLLECTOR_PORT"
fi
...
rollback_ipfix_enable() {
  ...
  if [ "$started" -eq 1 ]; then
    stop_project_process_if_present "$IPFIX_PID_FILE" softflowd \
      "$IPFIX_COLLECTOR_HOST:$IPFIX_COLLECTOR_PORT" || status=1
  fi
...
rm -f "$IPFIX_PID_FILE" "$IPFIX_LOG_FILE" "$IPFIX_COLLECTOR_RESULT" "$IPFIX_COLLECTOR_READY"
