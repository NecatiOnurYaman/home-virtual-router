...
The exact live argv is recorded in `/run/home-virtual-router/ipfix/command.txt`. Its final `-- ip` is an intentional IPv4 BPF expression, clearly separated from exporter options; no other trailing expression, sampling option, or capture-length override is used. The diagnostic tcpdump uses the same IPv4 selection and must prove it matches the dynamic client and upstream address before softflowd is evaluated. R8 retains `-N`; promiscuous mode and veth offloads are not changed without Ubuntu evidence that either is relevant.

The exporter enable check does not treat abbreviated `softflowctl -h` output as a command catalog. After the private socket appears, it queries the live namespace exporter with `softflowctl -c /run/home-virtual-router/ipfix/softflowd.ctl statistics`. This verifies that the binary and the exact project instance can communicate without touching a default host socket.

The isolated test collector runs only in `hvr-upstream`. It starts before fresh client traffic is generated. The test confirms the project exporter has tracked the dynamic client, then runs `softflowctl -c /run/home-virtual-router/ipfix/softflowd.ctl expire-all` inside `hvr-router`. The actual command exit status is authoritative; failure output is reported rather than falling back to another socket. Forced expiry makes softflowd export immediately and deterministically, so the test does not depend on a sleep long enough for natural expiry and leaves the exporter running. The lab exporter also uses short fallback expiry settings: `general=2`, `tcp=2`, `udp=2`, `maxlife=5`, and `expint=1` seconds.
...
Before querying live flows, `make ipfix-test` starts a PID-validated tcpdump inside `hvr-router`, captures only `hvr-lan` into `test-traffic.pcap`, and renders `tcpdump.txt`. The output must identify an Ethernet (`EN10MB`) datalink and contain both the DHCP client and `192.0.2.1`. A separate foreground, non-exporting `softflowd -r` invocation then reads that pcap with the same `ip` filter and writes `offline-softflowd.txt`; pcap-file mode uses neither a PID file nor a control socket. These artifacts distinguish no LAN visibility, an offline parser/datalink problem, and a live-capture problem. Only a positive live processed-packet count and tracked client flow permit `expire-all`.

Ubuntu softflowd 1.1.1 initializes live capture in this order: `pcap_create`, `pcap_set_snaplen`, `pcap_set_promisc`, `pcap_set_timeout(0)`, optional buffer sizing, and `pcap_activate`. It does not call `pcap_set_immediate_mode`, and its CLI exposes buffer size but not capture read timeout or immediate mode. The older 1.0.0 code used `pcap_open_live(..., timeout_ms=0)`, so the zero timeout itself was not newly introduced by the API migration. Current upstream retains the same create/timeout/activate sequence; no later upstream immediate-mode or positive-timeout fix has been identified. The project therefore does not patch or privately rebuild softflowd based only on this suspicion.

`make ipfix-live-diagnose` performs two temporary live captures on the same `hvr-lan` interface: first with `-N`, then with normal promiscuous capture. Each uses a unique project control socket, no collector, the same `ip` BPF, and a bounded burst of 2,048 ICMP requests with a 1,200-byte payload and 1 ms interval. It records package/libpcap versions and per-variant statistics under the IPFIX runtime directory, restores the original interface promiscuity count, and leaves the normal exporter running. Positive processing under `-N` after only the burst indicates buffering; zero under `-N` but positive promiscuous processing indicates a promiscuous-mode interaction; zero for both is recorded only as a D candidate pending a replacement-exporter decision. The diagnostic never captures or changes the UTM-facing interface.

R8 creates no routes, nftables rules, DHCP/DNS settings, host services, or VM-wide forwarding changes. The export datagrams are router-originated OUTPUT traffic, whereas the R5 nftables chain governs forwarded traffic; no forwarding rule is weakened for telemetry. Runtime PID, log, control socket, collector result, command, pcap, tcpdump, offline parser, statistics, tracked-flow, and expiry diagnostics are restricted to `/run/home-virtual-router/ipfix/`. Disable removes them with the rest of the exact project runtime state, stops only the PID-validated project exporter, and leaves R7 running; R7 teardown refuses while R8 is active.
...
- `make ipfix-live-diagnose` performs the bounded `-N`/promiscuous live-capture comparison and records package versions and statistics.
