...
HVR's dnsmasq configuration explicitly excludes loopback, so it does not claim router-local `127.0.0.1:53` or `[::1]:53`. An IPv6 link-local socket for the address assigned specifically to `hvr-lan` remains accepted as LAN binding behavior; it does not constitute general IPv6 routing or DNS support. R7 still rejects the router WAN address, `hvr-wan` link-local addresses, and IPv4/IPv6 wildcard listeners, and actively verifies from `hvr-upstream` that UDP and TCP DNS do not answer through `192.0.2.2`.
...
R8 uses Ubuntu's packaged `pmacctd` with its `nfprobe` plugin. Install it explicitly inside the VM with `sudo apt install pmacct`; project scripts never install packages or control a host pmacct service. Dependency checks query the installed binary's version, and enable proves the actual build can load `nfprobe` by starting the exact configuration and failing closed if the process exits or reports an unavailable plugin.

Pmacctd runs only inside `hvr-router` and captures the IPv4 BPF `ip` on `hvr-lan`. This LAN-side vantage point sees a client's original dynamic `10.0.0.x` source before R4 masquerading rewrites it on `hvr-wan`:

```text
hvr-client -- original source --> [hvr-lan | pmacctd/nfprobe] hvr-router [NAT | hvr-wan] --> hvr-upstream
                                      IPFIX v10 over UDP ---------------------------> 192.0.2.1:4739
```

The rendered configuration, pmacct-owned core PID file, command record, process-tree diagnostic, and log remain under `/run/home-virtual-router/ipfix/`. Pmacct runs a core process and a separate nfprobe plugin child. Lifecycle checks use pmacct's own PID, executable identity, direct parentage, recorded process start times, and `/proc/<pid>/ns/net` equality with `hvr-router`; they do not assume the background-launch PID or a rewritten process title is the core identity. The verified core is signalled first; a recorded plugin child is targeted separately only if it remains and its executable, namespace, and original process start time still prove project ownership. Failed enable preserves the configuration, command, log, and process tree while stopping only proven project processes. Explicit disable removes those diagnostics.

The named nfprobe instance aggregates source/destination address, ports, protocol, TOS, and ORed TCP flags. IPFIX supplies packet/octet counters plus absolute millisecond start/end timestamps (IE 152/153); `timestamps_secs` is deliberately not enabled. Ubuntu pmacct 1.7.8 accepts `nfprobe_engine` only for NetFlow v5, so the IPFIX configuration does not set it or fabricate an Observation Domain ID. The strict receiver records the exporter-defined value from actual IPFIX headers and rejects inconsistent values during one test. Pmacct's nfprobe represents each direction as an independent unidirectional record, matching the observability pipeline's independent-record semantics; R8 adds no deduplication.

The lab uses short expiry settings (`tcp=5`, `udp=3`, `icmp=3`, `general=3`, `maxlife=10`, `expint=1`) so a completed small flow appears within the receiver's bounded 12-second window. These are lab responsiveness settings; a physical deployment should reconsider the latency/data-volume tradeoff. `make ipfix-test` starts the collector first and generates only two ICMP requests, one UDP DNS exchange, and one TCP DNS exchange. It requires templates, data records, the standard fields, Observation Domain ID, and a record whose source is the current DHCP client, and records measured receiver latency in `collector-result.json`.

Softflowd 1.1.1-1build1 was evaluated and rejected for the normal R8 path. With Ubuntu 26.04's libpcap 1.10.6/TPACKET_V3, its live capture processed no low-volume packets even though tcpdump saw them and offline parsing succeeded; processing began only after substantial buffer fill, independently of promiscuous mode. Artificial traffic, buffer tricks, patching, and a private fork are intentionally not used. Disable retains only tightly scoped migration cleanup for a previous project PID and never touches `softflowd.service`.
