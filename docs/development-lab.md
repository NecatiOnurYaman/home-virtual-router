...
R8 creates no routes, nftables rules, DHCP/DNS settings, host services, or VM-wide forwarding changes. The export datagrams are router-originated OUTPUT traffic, whereas the R5 nftables chain governs forwarded traffic; no forwarding rule is weakened for telemetry. Runtime PID, log, control socket, collector result, `statistics.txt`, `dump-flows.txt`, and `expire-all.txt` are restricted to `/run/home-virtual-router/ipfix/`. These diagnostics distinguish control, capture, expiry, export, and receiver failures. Disable removes them with the rest of the exact project runtime state, stops only the PID-validated project exporter, and leaves R7 running; R7 teardown refuses while R8 is active.
