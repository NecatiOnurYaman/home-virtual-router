...
class PersistenceRenderingTests(unittest.TestCase):
    def test_unit_wraps_canonical_runtime_without_network_online(self) -> None:
        ...
        self.assertIn("After=network-pre.target NetworkManager.service systemd-networkd.service", unit)
        self.assertNotIn("After=docker.service", unit)
        self.assertNotIn("After=network-pre.target NetworkManager.service systemd-networkd.service docker.service", unit)
        self.assertNotIn("Wants=docker.service", unit)
